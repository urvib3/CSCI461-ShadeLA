...
def optimize_shade_placement(candidate_points, public_points, max_shades=15, spacing_threshold=500, public_service_threshold=300, use_spacing=True, use_public=True, use_heat=True, use_socioeconomic=True, weights=None):
    """
    ...
    # objective weights
    spacing_weight = weights['spacing']
    public_weight = weights['public']
    heat_weight = weights['heat']
    socioeconomic_weight = weights['socio']
    if use_spacing: 
        # encourage spacing (maximize distance between selected pairs)
        spacing_term = lpSum([
            y[(i,j)] * (-1 + (dist_matrix[i, j] / spacing_threshold))
