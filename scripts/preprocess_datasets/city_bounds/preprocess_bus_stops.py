...
output_path = os.path.join(output_dir, f"bus_stops_{target_city.lower()}.geojson")

os.makedirs(output_dir, exist_ok=True)

# ---------------------------------------------------------
# Load bus stops dataset
# ---------------------------------------------------------
...
if gdf.crs is None or gdf.crs.to_epsg() != 4326:
    print("⚙️ Reprojecting bus stops CRS to EPSG:4326...")
    gdf = gdf.to_crs(epsg=4326)

print(f"Total bus stops: {len(gdf)}")
...
# ---------------------------------------------------------
print(f"📍 Filtering bus stops inside {target_city}...")
gdf = gdf.to_crs(county_gdf.crs)  # ensure CRS match
gdf["in_city"] = gdf.geometry.within(city_geom)

city_bus_stops = gdf[gdf["in_city"]].drop(columns=["in_city"])

print(f"✅ Found {len(city_bus_stops)} bus stops in {target_city}.")

# ---------------------------------------------------------
# Save output
# ---------------------------------------------------------
city_bus_stops.to_file(output_path, driver="GeoJSON")
