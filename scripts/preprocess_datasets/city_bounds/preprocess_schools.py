...
# Paths and target
# ---------------------------------------------------------
input_path = "data/raw/la_schools_colleges_universities.geojson"
county_path = "data/raw/la_city_boundaries.geojson"
target_city = "Inglewood"  # update to desired city
output_dir = "preprocessed"
output_path = os.path.join(output_dir, f"schools_{target_city.lower().replace(' ', '_')}.geojson")
...
# ---------------------------------------------------------
print("🏫 Loading schools dataset...")
gdf = gpd.read_file(input_path)

# Ensure WGS84
if gdf.crs is None or gdf.crs.to_epsg() != 4326:
    print("⚙️ Reprojecting schools CRS to EPSG:4326...")
...
# ---------------------------------------------------------
print(f"📍 Filtering schools inside {target_city}...")
gdf = gdf.to_crs(county_gdf.crs)  # ensure CRS match
gdf["in_city"] = gdf.geometry.within(city_geom)

city_schools = gdf[gdf["in_city"]].drop(columns=["in_city"])

print(f"✅ Found {len(city_schools)} schools in {target_city}.")

# ---------------------------------------------------------
# Save output
# ---------------------------------------------------------
city_schools.to_file(output_path, driver="GeoJSON")
